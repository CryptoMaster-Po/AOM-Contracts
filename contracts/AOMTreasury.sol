...
contract AOMTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SAOM }
    ...
    address public sAOM;
    uint public sAOMQueue; // Delays change to sAOM address
    ...
    /**
        @notice allow approved address to deposit an asset for AOM
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AOM needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( AOM ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn AOM for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAOMERC20( AOM ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAOM ).balanceOf( msg.sender ); // Can only borrow against sAOM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AOM
        @param _amount uint
     */
    function repayDebtWithAOM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAOMERC20( AOM ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, AOM, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( AOM ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( AOM ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns AOM valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AOM decimals
            value_ = _amount.mul( 10 ** IERC20( AOM ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAOM ) { // 9
            sAOMQueue = block.number.add( blocksNeededForQueue );
        } else return false;
